...
// Our table component
function Table({ columns, data, setShowPopup, setTempResource }) {
...
function NavLinkButton({ row }) {
  console.log(row.original);

  return <Link to={`/resource/${row.original._id}`}>{row.original.name}</Link>;
}
...
function TableView() {
  ...
  const { fetchAllProviders } = useAppContext();
  const getData = async () => {
    try {
